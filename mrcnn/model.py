...
class MaskRCNN():
    ...
    def detect(self, images, verbose=0):
        ...
        if verbose:
            log("Processing {} imagesasdasd".format(len(images)))
            for image in images:
                log("image", image)
        ...
        return results
    def overlap_detection(self,rect1,rect2):
        try:
            ...
            return True
    def time_matrix_application(self,img,century,class_names):
        # Detection of object and receiveng probability of other classes 
        detection = self.detect([img],verbose=0) 
        r = detection[0]
        mrcnn = self.run_graph([img], [("probs", self.keras_model.get_layer("mrcnn_class").output)])
        
        # Time-matrix uploading and creating of list of probabilities according to classes 
        time_matrix=pd.read_csv("time_matrix.csv")
        print("INFO: Time-matrix was loaded correctly")
        ...
        #Class by class comapring with time matrix 
        for object_id in range(0,len(r["class_ids"])):
            ...
            if object_time.reset_index().loc[0,"Cent"]>century:
                    ...
                    if class_names[items[0]]==d_object:
                        for it in range(1,len(items)):
                            if items[it] not in r["class_ids"]:
                                ...
                                overlap_flag=False
                                for pos in position:
                                    if self.overlap_detection(r["rois"][pos],r["rois"][object_id])==True:
                                        overlap_flag=True
                                        #print(pos)
                ...
                print("INFO: Options for class ",d_object)
                ...
                print("INFO: Time-matrix didn't apply for class ",class_names[r["class_ids"][object_id]], ". ")
        return r
